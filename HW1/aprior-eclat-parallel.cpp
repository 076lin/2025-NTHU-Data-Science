...
void print4(ostream& out, const double number, const Itemset& item) {
    bool first = true;
    for (auto i : item) {
        if (!first) out << ',';
        out << i;
...
filter_frequencyTable(unordered_map<int, unordered_set<int>>& full_freq, const int& min_support) {
    unordered_map<int, unordered_set<int>> frequencyTable;

    for (auto& [key, value] : full_freq) {
        if ((int)value.size() >= min_support) {
...
generate_frequent_itemsets(
    ...
    for (const auto& [item, count] : single_freq) {
        if (count.size() >= min_support) {
            ...
            out << item << ':' << fixed << setprecision(4) << round_4(double(count.size()) / total_transactions) << '\n';
        }
    }
    ...
    while (!prev_frequent.empty()) {
        ...
        vector<pair<Itemset, unordered_set<int>>> local_results;

        vector<unordered_map<Itemset, unordered_set<int>, VectorHash>> thread_local_freq;
        vector<stringstream> thread_local_output;

        int num_threads = omp_get_max_threads();
        thread_local_freq.resize(num_threads);
        thread_local_output.resize(num_threads);
        ...
        for (int i = 0; i < (int)keys.size(); ++i) {
            ...
            for (int j = i + 1; j < (int)keys.size(); ++j) {
                ...
                const auto& tids_a = prev_frequent[a];
                const auto& tids_b = prev_frequent[b];
                const auto& smaller = (tids_a.size() < tids_b.size()) ? tids_a : tids_b;
                const auto& larger = (tids_a.size() < tids_b.size()) ? tids_b : tids_a;

                unordered_set<int> intersection;
                for (int id : smaller) {
                    if (larger.find(id) != larger.end()) {
                        intersection.emplace(id);
                    }
                }

                if ((int)intersection.size() >= min_support) {
                    thread_local_freq[tid][candidate] = intersection;
                    print4(thread_local_output[tid], double(intersection.size()) / total_transactions, candidate);
                }
            }
        }
